...
- **Error path.** `load_sales_data` raises `FileNotFoundError` or `ValueError` for any bad input: a missing column, an empty file, or a non-numeric value in `quantity`, `unit_price` or `total_amount`. `app.py` catches exactly those two exceptions, shows `st.error`, and calls `st.stop()`. To reject a new kind of bad data, raise `ValueError` in the loader; `app.py` then needs no change. By design the app never cleans or drops rows.
